window.SCRM = {
    ...
    getClubStatus: function(tbl, id, field) {
        ...
        return SCRM.clubStatus[key];
    },
    statusName: function(tbl, id, field) {
        return (SCRM.getClubStatus(tbl, id, field) || {}).name || '';
    },
    ...
    wssRes: {
        ...
        ace: ['https://cdnjs.cloudflare.com/ajax/libs/ace/1.4.2/ace.js'],
        summernoteBS4: [
...
club_Modal = function(linkData, handlers) {
    ...
    .on('hidden.bs.modal', function(e) {
        if (linkData.hideRemove) $(this).remove();
    })
    ...
    $.link(true, linkData.mdl, linkData, $.extend({
        on_OK: function(e, data) {
            ...
            if ( linkData.onOK(e, linkData) || linkData.okClose) linkData.mdl_hide();
        }
    }, handlers));
...
club_Drawer = function(linkData, handlers) {
    linkData.mdl_tpl = '#tpl_clubDrawer';
...
spinnr = function(show, elem) {
    ...
    var spelem = (elem)? elem.closest('.spinner-parent') : $('.spinner-parent');
...
detectSmartphone = function() {
    return (/iPhone|iPad|iPod|Android|webOS|BlackBerry|Windows Phone/i.test(navigator.userAgent) || screen.availWidth < 480);
},
...
$(function(){
    ...
    .on('shown.bs.tab', '[data-toggle="tab"].calculate', function(e) {
        var tab = $(e.target).trigger('calculate');
        console.log('shown.bs.tab.calculate', e, tab);
    })
    .on('calculate', '[data-toggle="tab"].calculate', function(e) {
        $(e.target).removeClass('calculate');
    })
    ...
    .on('click', '.telPopover', function(e){
        e.preventDefault();
        var lnk = $(this),
            tel = $.trim(lnk.data('tel')||lnk.text()),
            rows = [ ];
        
        if (!tel) {
            rows.push({text: SCRM.emptyStr, href: '#'});
        } else {
            var tels = tel.split(/[,;/]/);
            $.each(tels, function(i, tel) {
                tel = tel.match(/\d+/g).join('');
                if (tel.length == 11 && tel.slice(0,1) == '8') {
                    tel = '7' + tel.slice(tel.length - 10);
                }
                rows.push({ico: 'fa-phone', text: '+'+tel, href: 'tel:'+tel});
            });
            
            //https://api.whatsapp.com/send?phone={phone_number}&text={title}%20{url}
            rows.push({ico: 'fa-whatsapp', text: 'WhatsApp', href: 'https://api.whatsapp.com/send?phone='+tels[0]+'" target="_blank'});
            rows.push({ico: 'fa-comments-o', text: 'SMS', href: 'sms://'+ tels[0]});
            
            /*if ( detectSmartphone() ) {
                //sms:{phone_number}?body={url}{text}
                //+ makeDiv( makeExtLnk('<i class="fa fa-telegram"></i> Telegram', 'tg://resolve?domain=' + tel) );
            }*/
        }
        var d = {
            rows: rows,
        ...
        lnk.trigger('onInitTelPopover', d);
        if (d.rows.length) {
            var html = $.map(d.rows, function(v, i) {
                var ico = v.ico? '<div class="list-group-item-figure"><i class="fa '+v.ico+'"></i></div>' : '';
                return '<a href="'+v.href+'" class="list-group-item list-group-item-action">'+ ico +
                    '<div class="list-group-item-body">'+ v.text +'</div></a>';
            }).join('');
            clubOnPopover(lnk, '<div class="list-group list-group-divider">'+html+'</div>').find('.popover-body').addClass('p-0');
        }
    })
    ...
    .on('click', '.club-file,.club-img', function (e) {
        var href = $(this).attr('href'),
            ext = href.split('.').pop();
        if ($.inArray(ext, ['jpg','jpeg','png','gif','heic']) != -1) {
            e.preventDefault();
            showPhotos(href);
        }
    })
    ...
    .on('click', '[data-hint]', function(e){
        ...
        pJSON('/data/hint/'+hint_key, function(data){
            if (data) {
                if (data.info) {
                    club_Modal({
                        body: data.info,
                    ...
                    clubOnPopover(lnk, data.name);
                }
            }
        });
    })
    .on('click', '.nyet', function(e) {
        e.preventDefault();
        SCRM.alert("Действие пока не доступно. Скоро!");
    })
        ...
        function _initDateEdit(){
            el.each(function() {
                ...
                if (options.clearButton) {
                    el.wrap('<div class="input-group has-clearable flatpickr"></div>');
                ...
                el.trigger('onInitDateEdit');
            })
            .addClass('removeOnClose-flatpickr')
        }

        if (!window.flatpickr) SCRM.loadWSS('flatpickr', function(){
            _initDateEdit();
        }); else _initDateEdit();
        return el;
    };
        ...
        return this.each(function() {
            ...
            if (!elm.data('editMode')) {
                var etext = $('.roinplacetext', elm),
                    ...
                    roInplaceInner = $('<div class="roinplace-container"></div>').insertAfter( elm.hide().data('editMode', 1) );
                if (txt == SCRM.emptyStr) txt = '';
        
                $.templates('#roInplaceHtml').link(roInplaceInner, {
                    inplace_value: txt,
                    old_inplace_value: txt
                }, {
                    InplaceSubmit: function(e, data) {
                        e.preventDefault(); // submit reset
                        var d = data.linkCtx.data,
                            ev = elm.data('roinplace') || 'roinplace';
                        if (e.type=='submit') {
                            ((etext.length>0)?etext:elm).text(d.inplace_value);
                            if (callback) callback(d.inplace_value);
                            elm.trigger(ev, d.inplace_value);
                        }
                        $('.removeOnClose-flatpickr', roInplaceInner).destroyDateEdit();
                        roInplaceInner.remove();
                        elm.show().data('editMode', null);
                    }
                });
                var input = $('input', roInplaceInner)
                .on('keyup', function(event) {
                    if (event.key == "Escape") {
                        $(this).closest('form')[0].reset();
                    }
                });
                var editor = (elm.data('editor')||'').split(':');
                switch (editor[0]) {
                    case 'mask':
                    ...
                    break;
                    case 'dateedit': 
                        input.on('onInitDateEdit', function(e){
                            input.pauseFocus();
                        })
                        .initDateEdit();
                    break;
...
$.templates({
    ...
    '#tpl_Select' : '<select class="custom-select" data-link="{:~link:}">{^{for ~rows}}<option value="{{:id}}">{{:name}}</option>{{/for}}</select>',
